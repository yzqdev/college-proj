...
public class TeacherServiceImpl implements TeacherService {
    ...
    */
    ...
    public int addTeacher(Teacher teacher) throws Exception{
        ...
        //未上传图片
        if (picture_id==null||picture_id.trim().isEmpty()){
            return 1;
        }
    ...
    */
    @Override
    public List<Teacher> getTeachersByCourse(Course course) throws Exception{

        List<Teacher> teachers=new ArrayList<>();

        String [] teacher_ids=teacher_courseDao.getCurrentTeacherIdByCourse(course.getId());
        if (teacher_ids==null){
    ...
    */
    @Override
    public int uploadTeacherPicture(String teacher_id,String picture_id) throws Exception{

        String currentPicture_id=teacher_pictureDao.getCurrentPictureIdByTeacher(teacher_id);
        //当前没有课程图片
        if (currentPicture_id==null){

            int addTeacherPictureSuccess;
            addTeacherPictureSuccess=teacher_pictureDao.addTeacher_picture(teacher_id,picture_id);
            if (addTeacherPictureSuccess!=1){
                throw new RuntimeException("上传教师图片失败");
            }

        }
        //先前存在教师的图片，现在进行修改，即上传了一张新的图片
        else{
            ...
            deleteRelationSuccess=teacher_pictureDao.deleteTeacher_picture(teacher_id);
            if (deleteRelationSuccess!=1){
                throw new RuntimeException("删除教师图片关系失败");
            }

...
public class RelationServiceImpl implements RelationService {
    ...
    public Integer updateRelation_courseGroup_course(Course course) throws Exception {
        ...
        if (currentCourseGroup_id == null) {
            ...
            addCourseGroup_courseSuccess = courseGroup_courseDao.addRelation(courseGroup_id, course_id);
            ...
            for (String teacher_id : teacher_ids) {
                ...
                for (String courseId : course_ids) {
                    ...
                    } else if (courseGroupId.equals(currentCourseGroup_id)) {
                        count++;
                    }
    ...
    @Override
    @Transactional
    public Video bindUnit_video(String unit_id, String video_id) throws Exception {

        Video video;

        video = videoDao.queryVideoById(video_id);
        if (video == null) {
            throw new RuntimeException("视频不存在");
        }

        int addUnit_VideoSuccess;
        addUnit_VideoSuccess = unit_videoDao.addRelation(unit_id, video_id);
        if (addUnit_VideoSuccess != 1) {
            throw new RuntimeException("添加教学单元教学资料关联失败");
        }
    ...
    */
    @Override
    @Transactional
    public Office bindCourse_office(String course_id, String office_id) throws Exception {

        Office office;

        office = officeDao.queryOfficeById(office_id);

        if (office == null) {
            throw new RuntimeException("教学资料不存在");
        }

        int addCourse_OfficeSuccess;
        addCourse_OfficeSuccess = course_officeDao.addRelation(course_id, office_id);
        if (addCourse_OfficeSuccess != 1) {
            throw new RuntimeException("添加课程课程资料关联失败");
        }

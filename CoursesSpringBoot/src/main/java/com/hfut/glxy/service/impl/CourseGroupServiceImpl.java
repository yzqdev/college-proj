...
public class CourseGroupServiceImpl implements CourseGroupService {
    ...
    */
    ...
    public int deleteCourseGroup(String courseGroup_id) throws Exception {
        ...
        for (String course_id:course_ids){
            try {
                courseService.putToDustbin(course_id);
            }catch (Exception e){
                throw new RuntimeException("删除课程出现错误");
            }
        }
    ...
    */
    @Override
    public int uploadCourseGroupPicture(String courseGroup_id,String picture_id) throws Exception{

        String currentPicture_id=courseGroup_pictureDao.getCurrentPictureIdByCourseGroup(courseGroup_id);
        //当前没有课程图片
        if (currentPicture_id==null){

            int addCourseGroupPictureSuccess;
            addCourseGroupPictureSuccess=courseGroup_pictureDao.addCourseGroup_picture(courseGroup_id,picture_id);
            if (addCourseGroupPictureSuccess!=1){
                throw new RuntimeException("上传课程图片失败");
            }

        }
        //先前存在课程的图片，现在进行修改，即上传了一张新的图片
        else{
            ...
            //删除原先的图片
            int deleteCurrentPictureSuccess;
            deleteCurrentPictureSuccess=pictureDao.deletePictureById(currentPicture_id);
            if (deleteCurrentPictureSuccess!=1){
                throw new RuntimeException("删除图片出现错误");
            }

            //删除课程与原先图片的关系
            int deleteRelationSuccess;
            deleteRelationSuccess=courseGroup_pictureDao.deleteRelationByCourseGroup_Teacher(courseGroup_id,currentPicture_id);
            if (deleteRelationSuccess!=1){
                throw new RuntimeException("删除课程组图片关系失败");
            }
            ...
            addCourseGroupPictureSuccess=courseGroup_pictureDao.addCourseGroup_picture(courseGroup_id,picture_id);
            if (addCourseGroupPictureSuccess!=1){
                throw new RuntimeException("增加图片出现错误");
            }

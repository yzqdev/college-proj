...
*/
    ...
    */
    @ResponseBody
    @RequestMapping(value = "/uploadCourseGroupPicture",method = RequestMethod.POST)
    public Result<String> uploadCoursePicture(@RequestBody Map map){

        String courseGroup_id=(String)map.get("courseGroup_id");
        String picture_id=(String)map.get("picture_id");

        if (courseGroup_id==null||picture_id==null){
            return new Result<>(false,"未选择课程组或未上传图片",null);
        }
        ...
        try{
            ...
            success=courseGroupService.uploadCourseGroupPicture(courseGroup_id,picture_id);
            if (success==0){

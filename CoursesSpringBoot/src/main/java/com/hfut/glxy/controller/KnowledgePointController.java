...
public class KnowledgePointController {
    ...
    */
    ...
    public Result<List<KnowledgePoint>> getKnowledgePointsByUnit(@RequestBody Map map){
    ...
    */
    @ResponseBody
    @RequestMapping(value = "/addKnowledgePoints",method = RequestMethod.POST)
    public Result<String> addKnowledgePoints(@RequestBody Map map){
        ...
        String knowledgePoints=(String)map.get("knowledgePoints");
        if (knowledgePoints==null||knowledgePoints.trim().isEmpty()){
            return new Result<>(false,"知识点不能为空",null);
        }

        int success;

        try{
            success=knowledgePointService.addKnowledgePoints(unit_id,knowledgePoints);
        }catch (Exception e){
            e.printStackTrace();
            return new Result<>(false,"批量添加知识点出现异常",null);
        }

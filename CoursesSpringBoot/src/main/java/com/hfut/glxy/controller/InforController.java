...
public class InforController {
    ...
    */
    @ResponseBody
    @RequestMapping(value = "/getCoursesByCourseGroup",method = RequestMethod.POST)
    public Result<Map> getCoursesByCourseGroup(@RequestBody CourseGroup courseGroup){

        String courseGroup_id=courseGroup.getId();
        if (courseGroup_id==null||courseGroup_id.trim().isEmpty()){
            return new Result<>(false,"课程组id未获取",null);
        }

        Map map;

        try{
            map=inforService.getCoursesByCourseGroup(courseGroup_id);
        }catch (Exception e){
            e.printStackTrace();
            return new Result<>(false,"获取异常",null);
        }

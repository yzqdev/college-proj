...
public class TeacherController {
    ...
    */
    public Result<String> addTeacher(@RequestBody Teacher teacher){
        ...
        if (teacher.getNumber()==null||teacher.getNumber().trim().isEmpty()){
            return new Result<>(false,"教师编号不能为空",null);
        }
        if (teacher.getName()==null||teacher.getNumber().trim().isEmpty()){
            return new Result<>(false,"教师姓名不能为空",null);
        }
        if (teacher.getSex()==null||teacher.getNumber().trim().isEmpty()){
            return new Result<>(false,"教师性别不能为空",null);
        }
    ...
    */
    @ResponseBody
    @RequestMapping(value = "/uploadTeacherPicture",method = RequestMethod.POST)
    public Result<String> uploadTeacherPicture(@RequestBody Map map){

        String teacher_id=(String)map.get("teacher_id");
        String picture_id=(String)map.get("picture_id");

        if (teacher_id==null||picture_id==null){
            return new Result<>(false,"未选择教师或未上传图片",null);
        }
        ...
        try{
            ...
            success=teacherService.uploadTeacherPicture(teacher_id,picture_id);
            if (success==0){
                return new Result<>(false,"gg,未知错误",null);
            }

        }catch (Exception e){
            e.printStackTrace();
            return new Result<>(false,"绑定失败，请重试",null);
        }

        return new Result<>(true,"绑定成功",null);

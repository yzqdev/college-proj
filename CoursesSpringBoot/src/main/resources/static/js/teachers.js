  ...
  function successFunction(data) {
    ...
    if (data.success) {
      for (let i in data.data) {
        $(".div-body .body-left ul").append(
          `<li id="group-${i}" index="${data.data[i].courseGroup.id}"><i class="glyphicon glyphicon-unchecked"></i>${data.data[i].courseGroup.name}</li>`
        );
        $(".div-body .body-left").after(
          `<div class="body-right group-${i}"></div>`
        );
        console.log(1);

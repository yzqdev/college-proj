      ...
      columnDefs: [
        {
          ...
          render: function (data, type, row, meta) {
            ...
            var allCourseGroupSelect =
              '<select class="form-control" id="courseGroupSelect' +
              courseGroupSelectNameCount +
              '">';

            if (data2.data.courseGroups == null) {
            } else {
              for (var i = data2.data.courseGroups.length - 1; i >= 0; i--) {
                try {
                  if (data2.data.courseGroups[i].id == row.courseGroup.id) {
                    allCourseGroupSelect +=
                      "<option selected value='" +
                      data2.data.courseGroups[i].id +
                      "'>" +
                      data2.data.courseGroups[i].name +
                      "</option>";
                  } else {
                    allCourseGroupSelect +=
                      "<option value='" +
        ...
        },
        {
          ...
          render: function (data, type, row, meta) {
            ...
            var allTeachersSelect =
              '<select class="selectpicker show-menu-arrow form-control" ' +
              'multiple="multiple" id="teacherSelect' +
              nameCount +
              '">';

            console.log("data2: " + data2);
            console.log(data2.data.teachers);

            if (data2.data.teachers == null) {
            } else {
              for (var i = data2.data.teachers.length - 1; i >= 0; i--) {
                allTeachersSelect +=
                  "<option value='" +
                  data2.data.teachers[i].id +
                  "'>" +
                  data2.data.teachers[i].name +
                  "</option>";
              }
            }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $(".selectpicker").each(function () {
          ...
          if (data.data.data[item].teachers == null) {
          } else {
            for (
              var j = data.data.data[item].teachers.length - 1;
              j >= 0;
              j--
            ) {

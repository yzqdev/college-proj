...
$(document).ready(function () {
  ...
  function functionSuccy(data) {
    ...
    if (data.success) {
      ...
      $(".course-intro")
        .before("a")
        .append(`简介：${data.data.course.introduction}&nbsp;&nbsp;&nbsp;`);
      ...
      $(".begin a").attr(
        "href",
        `video.html?id=${data.data.chapters[0].units[0].id}`
      );
      ...
      $(".teacher-name").html(
        `讲师介绍：${data.data.teacherDetail.teacher.name}`
      );
      $(".teacher-intro").html(
        `简介：${data.data.teacherDetail.teacher.personIntroduction}`
      );
      ...
      for (i in data.data.chapters) {
        $(".subNavBox").append(
          `<div class="subNav">${data.data.chapters[i].chapter.number}:${data.data.chapters[i].chapter.name}</div><ul class="navContent ul-${i}"></ul> `
        );
        // let a = i
        for (x in data.data.chapters[i].units) {
          ...
          $(".subNavBox")
            .find(`.ul-${i}`)
            .append(
              `<li><a href="video.html?id=${data.data.chapters[i].units[x].id}">${data.data.chapters[i].units[x].number}:${data.data.chapters[i].units[x].name}</a></li>`
            );
        }
      }
      // 加载课程资料
        ...
        if (data.success) {
          ...
          for (let i of data.data) {
            $("#myModal modal-body div").append(
              `<li style="list-style:none;margin-left:20px;margin-top:20px;font-size:16px;"><a href="video.html?id=${i.unit.id}">${i.course.name}&nbsp;&nbsp;${i.chapter.number}&nbsp;&nbsp;${i.unit.number}&nbsp;&nbsp;${i.unit.name}</a></li>`
            );
          }
        } else {
          $("#myModal iframe").css("display", "none");
